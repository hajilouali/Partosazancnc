$(function () {
    $.get("/Admin/Contactus/ListResult",
        function (result) {
...
function pageid(id) {
    ...
    $.get("/Admin/Contactus/ListResult?pageId="  + id,
        function (result) {
            $("#Comment_content").html(result);
        });
...
function Edit(id) {
    $.get("/Admin/Contactus/CommentEdite/" + id,
        function (result) {
...
function PublishComment(id) {
    $.get('/Admin/Contactus/PublishComment/' + id,
        function () {
            $("#item_" + id).removeClass();
            $("#itempub_" + id).remove();
            $("#btnitem_" + id).append('<a id="itemdraf_' +
...
function DraftComment(id) {
    $.get('/Admin/Contactus/DraftComment/' + id,
        function () {
...
function DeleteComment(id) {
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.get('/Admin/Contactus/DeleteComment/' + id,
                    function () {
                        $("#item_" + id).hide('slow');
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $(chkArray).each(function () {
                        $.get('/Admin/Contactus/PublishComment/' + this,
                            function () {
    ...
    if (chkArray.length > 0) {
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $(chkArray).each(function () {
                        $.get('/Admin/Contactus/DraftComment/' + this,
                            function () {
    ...
    if (chkArray.length > 0) {
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $(chkArray).each(function () {
                        $.get('/Admin/Contactus/DeleteComment/' + this,
                            function () {
